...
export default function UsersList({ usuario, onUserUpdated }) {
  ...
  const handleEdit = () => {
    ...
    setSnackbar({
      open: true,
      message: `Edição do usuário ${user.name} (em desenvolvimento)`,
      severity: 'info'
    })
  }
  ...
  const handleToggleActivated = async () => {
    ...
    try {
      ...
      const url = `http://localhost:3000/usuarios/${userId}` // Ajuste conforme seu endpoint
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          // Se necessário, inclua outros campos que seu backend espera
          username: user.username,
          // password não deve ser enviado aqui
        })
      })
      ...
      setSnackbar({
        open: true,
        message: `Usuário ${user.name} ${newActivatedStatus ? 'ativado' : 'desativado'} com sucesso!`,
        severity: 'success'
      })
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/deletarUsuario/${userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
    ...
    <>
      <UserCard>
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={2}>
            <Tooltip title={user.activated ? "Usuário ativo" : "Usuário inativo"}>
          ...
          <Grid item xs={2} sx={{ textAlign: 'right' }}>
            {checked && (
              <ButtonGroup variant="outlined" size="small" disabled={loading || updating}>
                ...
                <Tooltip title="Editar">
                  <Button onClick={handleEdit} disabled={loading || updating}>
        ...
        {checked && user.id && (
          <Box sx={{ mt: 2, pt: 2, borderTop: '1px dashed #e0e0e0' }}>
            <Grid container spacing={1}>
              ...
              <Grid item xs={4}>
                <Typography variant="caption" color="text.secondary">
                  {user.createdAt ? `Criado: ${new Date(user.createdAt).toLocaleDateString()}` : ''}
                </Typography>
              </Grid>
            </Grid>
          </Box>
