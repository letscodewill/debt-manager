...
export default function TransitionsModalEdit({icon, divida}) {
  const { category, setCategory, fetchDespesas, setDespesas } =
    useContext(Context)
  const { token, user, logout } = useContext(AuthContext)

  const [open, setOpen] = React.useState(false)
  const [descricao, setDescricao] = React.useState(divida.descricao)
  const [valor, setValor] = React.useState(divida.valor)
  const [categoria, setCategoria] = React.useState(divida.categoria)
  ...
  const handleOpen = () => {
    if (!token) {
      setSnackbar({
        open: true,
        message: 'Você precisa estar logado para editar uma dívida!',
        severity: 'warning'
      })
      return
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/dividas/${divida.id}`, {
        method: 'PUT',
        headers: {
      ...
      setSnackbar({
        open: true,
        message: `Erro ao editar: ${error.message}`,
        severity: 'error'
      })
